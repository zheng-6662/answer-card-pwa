...
当前相机扫描会优先找选择题区域外面的黑色矩形边框，也就是选择题区域的四个角。找到后会先自动裁出选择题区域，再按固定模板识别 55 个选项；只有外框没有找到或网格校验不够稳时，才会回到旧的多候选识别流程。

使用时不需要额外贴黑块，但要尽量让选择题区域外框的四个角都进入相机画面。白色取景框只作为对准提示，程序会使用完整相机预览画面识别，避免因为答题卡贴近白框边缘而被硬裁掉。

如果以后可以改答题卡模板，建议在选择题区域四个角加实心黑色定位块，识别速度和稳定性还能继续提升。
